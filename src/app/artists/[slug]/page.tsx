...
import { BadgeCheck, Film, Store, CalendarDays, Mail, Trophy } from "lucide-react";
...
function catClass(c: string | null) {
  return c ? (categoryColors[c.toLowerCase()] ?? "text-zinc-400 bg-zinc-400/10 border-zinc-400/20") : "text-zinc-400 bg-zinc-400/10 border-zinc-400/20";
}
  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-white">
      ...
      {artist.artist_of_month && (
        <div className="w-full bg-gradient-to-r from-amber-600/20 to-amber-500/10 border-b border-amber-500/20 px-6 py-3 flex items-center justify-center gap-2">
          <Trophy className="w-4 h-4 text-amber-400" />
          <span className="font-manrope text-sm text-amber-300 font-semibold">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-16 flex flex-col md:flex-row items-center md:items-end gap-8">
          ...
          <div className="shrink-0">
            {artist.avatar_url ? (
              <img src={artist.avatar_url} alt={artist.name} className="w-24 h-24 rounded-full object-cover ring-2 ring-white/10" />
            ) : (
              <div className="w-24 h-24 rounded-full bg-zinc-800 ring-2 ring-white/10 flex items-center justify-center">
                <span className="font-raleway text-3xl font-bold text-zinc-300">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 flex-wrap">
              <h1 className="font-raleway text-4xl font-bold text-white">{artist.name}</h1>
              {artist.verified && (
                <span className="inline-flex items-center gap-1 text-[11px] font-manrope font-semibold uppercase tracking-widest text-fuchsia-400 bg-fuchsia-400/10 border border-fuchsia-400/20 px-2 py-0.5 rounded">
                  <BadgeCheck className="w-3.5 h-3.5" /> Verified
                </span>
              )}
            </div>
            {artist.bio && (
              <p className="font-manrope text-zinc-400 mt-2 max-w-xl">{artist.bio}</p>
            )}
            {artist.contact_email && (
              <a
                href={`mailto:${artist.contact_email}`}
                className="mt-3 inline-flex items-center gap-1.5 text-sm font-manrope text-fuchsia-400 hover:text-fuchsia-300 transition-colors"
              >
      ...
      <div className="border-y border-white/[0.06] bg-white/[0.02]">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center gap-8 flex-wrap">
          {[
            ...
            <div key={label} className="flex items-center gap-2 text-sm font-manrope text-zinc-400">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        {(shows ?? []).length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            ...
            <p className="font-raleway text-white font-semibold text-lg">No published shows yet</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {(shows ?? []).map((show: {
              ...
              <Link
                key={show.id}
                href={`/shows/${show.id}`}
                className="group relative flex flex-col rounded-xl overflow-hidden border border-zinc-800 bg-zinc-900/60 hover:border-zinc-700 transition-colors"
              >
                <div className="relative aspect-video bg-zinc-800 overflow-hidden">
                  ...
                  {show.featured && (
                    <div className="absolute top-2.5 right-2.5">
                      <span className="inline-block text-[10px] font-manrope font-semibold uppercase tracking-widest px-2 py-0.5 rounded border text-fuchsia-400 bg-fuchsia-400/10 border-fuchsia-400/30">
                        Featured
                      </span>
                    </div>
                  )}
                ...
                <div className="p-4">
                  <h3 className="font-raleway font-semibold text-white text-sm leading-snug">{show.title}</h3>
                  {show.description && (
                    <p className="font-manrope text-xs text-zinc-400 mt-1.5 line-clamp-2">{show.description}</p>
                  )}
                </div>
              </Link>
            ))}
