...
export default function BoostFlow({ shows, preselectedShowId, success }: Props) {
  ...
  if (success || step === 3) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center px-8 text-center gap-6">
        ...
        <div>
          <h1 className="font-raleway text-2xl font-semibold text-white">Your show is now featured.</h1>
          <p className="font-manrope text-sm text-zinc-400 mt-2 max-w-sm">
  ...
  return (
    <div className="px-8 py-8 max-w-3xl mx-auto w-full">
      {/* Header */}
      <div className="mb-8">
        <Link href="/dashboard/artist" className="inline-flex items-center gap-1.5 text-sm font-manrope text-zinc-500 hover:text-zinc-300 transition-colors mb-4">
          <ArrowLeft className="w-4 h-4" /> Back to Studio
        </Link>
        <h1 className="font-raleway text-2xl font-semibold text-white">Boost Your Show</h1>
        <p className="font-manrope text-sm text-zinc-400 mt-1">Get featured placement in the venue show library.</p>
      </div>
        ...
        {["Placement", "Duration", "Payment"].map((label, i) => (
          <div key={label} className="flex items-center gap-2">
            <div className={`flex items-center justify-center w-7 h-7 rounded-full text-xs font-manrope font-semibold ${
              step > i ? "bg-fuchsia-600 text-white" : step === i ? "bg-fuchsia-600/20 border border-fuchsia-500/50 text-fuchsia-300" : "bg-zinc-800 text-zinc-600"
            }`}>
              {step > i ? <Check className="w-3.5 h-3.5" /> : i + 1}
            </div>
            <span className={`text-xs font-manrope ${step >= i ? "text-zinc-300" : "text-zinc-600"}`}>{label}</span>
            {i < 2 && <div className={`w-8 h-px ${step > i ? "bg-fuchsia-600/50" : "bg-zinc-800"}`} />}
          </div>
        ))}
      </div>
      ...
      {step === 0 && (
        <div className="flex flex-col gap-4">
          <h2 className="font-raleway text-lg font-semibold text-white mb-2">Choose placement</h2>
          {PLACEMENTS.map(({ type, label, price, desc, icon: Icon }) => (
            ...
            >
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${placement === type ? "bg-fuchsia-500/15" : "bg-zinc-800"}`}>
                  <Icon className={`w-6 h-6 ${placement === type ? "text-fuchsia-400" : "text-zinc-500"}`} />
                </div>
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <p className="font-raleway font-semibold text-white text-base">{label}</p>
                    <p className="font-raleway text-xl font-bold text-white">€{price}<span className="text-sm font-manrope text-zinc-400">/mo</span></p>
                  </div>
                  <p className="font-manrope text-sm text-zinc-400 mt-1">{desc}</p>
                </div>
              </div>
            </button>
          ))}
                ...
                {shows.map((show) => (
                  <button
                    ...
                    className={`flex items-center gap-3 p-3 rounded-lg border transition-all text-left ${
                      selectedShowId === show.id ? "border-fuchsia-500/50 bg-fuchsia-500/5" : "border-zinc-800 bg-zinc-900/40 hover:border-zinc-700"
                    }`}
                  >
                    <div className="w-12 h-8 rounded overflow-hidden bg-zinc-800 shrink-0">
                      {show.thumbnail_url
                    ...
                    <span className="font-manrope text-sm text-white">{show.title}</span>
      ...
      {step === 1 && (
        <div className="flex flex-col gap-4">
          <h2 className="font-raleway text-lg font-semibold text-white mb-2">Choose duration</h2>
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-2 p-5 rounded-xl border transition-all ${
                    months === m ? "border-fuchsia-500/60 bg-fuchsia-500/5" : "border-zinc-800 bg-zinc-900/60 hover:border-zinc-700"
                  }`}
                >
                  <span className="font-raleway font-semibold text-white text-base">{label}</span>
                  <span className="font-raleway text-xl font-bold text-white">€{price.toFixed(0)}</span>
                  {discount > 0 && (
                    <span className="text-[11px] font-manrope font-semibold text-emerald-400 bg-emerald-400/10 border border-emerald-400/20 px-2 py-0.5 rounded">
                      {discount}% off
                    </span>
                  )}
                </button>
          ...
          <div className="p-4 rounded-xl border border-zinc-800 bg-zinc-900/40 mt-2">
            <div className="flex items-center justify-between">
              <span className="font-manrope text-sm text-zinc-400">Total</span>
              <span className="font-raleway text-2xl font-bold text-white">€{total.toFixed(2)}</span>
            </div>
          </div>

          <div className="flex gap-3 mt-2">
            <button type="button" onClick={() => setStep(0)} className="flex-1 py-3 rounded-lg border border-zinc-800 bg-zinc-900 text-zinc-300 font-manrope font-semibold hover:border-zinc-700 transition-colors">
              Back
            </button>
      ...
      {step === 2 && (
        <div className="flex flex-col gap-4">
          <h2 className="font-raleway text-lg font-semibold text-white mb-2">Review & pay</h2>
          <div className="p-6 rounded-xl border border-zinc-800 bg-zinc-900/40 flex flex-col gap-4">
            <div className="flex items-center justify-between text-sm font-manrope">
              <span className="text-zinc-400">Placement</span>
              <span className="text-white">{selectedPlacement?.label}</span>
            </div>
            <div className="flex items-center justify-between text-sm font-manrope">
            ...
            {selectedDuration.discount > 0 && (
              <div className="flex items-center justify-between text-sm font-manrope">
                ...
                <span className="text-emerald-400">−{selectedDuration.discount}%</span>
              </div>
            )}
          ...
          <div className="flex gap-3 mt-2">
            <button type="button" onClick={() => setStep(1)} className="flex-1 py-3 rounded-lg border border-zinc-800 bg-zinc-900 text-zinc-300 font-manrope font-semibold hover:border-zinc-700 transition-colors">
              Back
            </button>
            <button
              type="button"
