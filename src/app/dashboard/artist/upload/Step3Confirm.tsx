    ...
    <div className="flex flex-col gap-7">
      ...
      <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 overflow-hidden">
        ...
        <div className="px-5 py-4 flex flex-col gap-3">
          <div>
            <p className="font-raleway text-base font-semibold text-zinc-900 dark:text-white">{details.title}</p>
            {details.category && (
              <span className="inline-block mt-1 font-manrope text-[11px] text-zinc-600 dark:text-zinc-400 uppercase tracking-widest border border-zinc-200 dark:border-zinc-700 rounded px-2 py-0.5">
                {details.category}
              </span>
            )}
          </div>
          {details.description && (
            <p className="font-manrope text-xs text-zinc-600 dark:text-zinc-400 leading-relaxed">{details.description}</p>
          )}
          {details.tags.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {details.tags.map((t) => (
                <span key={t} className="font-manrope text-[11px] text-zinc-500 bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded">
                  {t}
                </span>
              ))}
            </div>
          ...
          {(Object.entries(fileLabels) as [FileKey, typeof fileLabels[FileKey]][]).map(([key, { label, icon: Icon, required }]) => {
            ...
            return (
              <div key={key} className="flex items-center gap-3 px-5 py-3.5">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${uploaded ? "bg-emerald-500/10 border border-emerald-500/20" : "bg-zinc-100 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700"}`}>
                  <Icon className={`w-4 h-4 ${uploaded ? "text-emerald-500 dark:text-emerald-400" : "text-zinc-400 dark:text-zinc-500"}`} />
                </div>
                ...
                {uploaded ? (
                  ...
                  <span className="font-manrope text-xs text-zinc-400 dark:text-zinc-600">Not uploaded</span>
                )}
              </div>
            );
          })}
            ...
            <button
              ...
              className={`w-5 h-5 mt-0.5 rounded border flex items-center justify-center shrink-0 transition-all ${
                checked
                  ...
                  : "bg-white dark:bg-zinc-900 border-zinc-300 dark:border-zinc-700 group-hover:border-zinc-400 dark:group-hover:border-zinc-500"
              }`}
            >
              {checked && <CheckCircle2 className="w-3 h-3 text-white" />}
            </button>
            <span className="font-manrope text-sm text-zinc-600 dark:text-zinc-400 leading-relaxed group-hover:text-zinc-800 dark:group-hover:text-zinc-300 transition-colors">
              {label}
            </span>
          </label>
        ))}
