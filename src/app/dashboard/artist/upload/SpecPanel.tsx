  ...
  return (
    <div className="px-7 py-8 flex flex-col gap-7">
      ...
      <div className="flex items-center gap-2.5">
        <div className="w-7 h-7 rounded-md bg-zinc-100 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 flex items-center justify-center shrink-0">
          <Monitor className="w-3.5 h-3.5 text-zinc-500 dark:text-zinc-400" />
        </div>
        <h2 className="font-raleway text-sm font-semibold text-zinc-900 dark:text-white tracking-wide">
      ...
      <div className="flex flex-col gap-1">
        ...
        {specs.map(({ label, value }) => (
          <div key={label} className="flex flex-col py-2 border-b border-zinc-100 dark:border-zinc-900 last:border-0">
            <span className="font-manrope text-[11px] text-zinc-500">{label}</span>
            <span className="font-manrope text-xs text-zinc-800 dark:text-zinc-200 font-medium mt-0.5">{value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-amber-500/25 bg-amber-500/5 p-4 flex flex-col gap-2.5">
        <div className="flex items-center gap-2">
          ...
          <span className="font-manrope text-[11px] font-semibold text-amber-500 dark:text-amber-400 uppercase tracking-widest">
            Design Tip
          </span>
        </div>
        <p className="font-manrope text-xs text-amber-700/80 dark:text-amber-200/70 leading-relaxed">
      ...
      {/* Version note */}
      <p className="font-manrope text-[10px] text-zinc-400 dark:text-zinc-600 leading-relaxed">
