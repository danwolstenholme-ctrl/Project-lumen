...
import {
  ...
  BadgeCheck, ExternalLink, DollarSign, Users, Store, Pencil, X, Check,
  Sparkles, Trash2, BarChart2, XCircle,
} from "lucide-react";
...
const statusConfig = {
  ...
  draft:     { label: "Draft",     icon: AlertCircle,   cls: "text-zinc-400 bg-zinc-400/10 border-zinc-400/20" },
...
export default function ArtistStudio({
  ...
  const stats = [
    ...
    { label: "This Month",        value: fmt(monthEarnings), icon: DollarSign, color: "text-emerald-400", wide: true },
  ];
  ...
  return (
    <div className="px-8 py-8 max-w-7xl mx-auto w-full">
      ...
      {/* Artist Header */}
      <div className="flex items-start gap-5 mb-8 p-6 rounded-2xl border border-zinc-800 bg-zinc-900/40">
        {/* Avatar */}
        <div className="shrink-0">
          {userImage ? (
            <img src={userImage} alt={userName} className="w-16 h-16 rounded-full object-cover ring-2 ring-zinc-700" />
          ) : (
            <div className="w-16 h-16 rounded-full bg-zinc-800 ring-2 ring-zinc-700 flex items-center justify-center">
              <span className="font-raleway text-xl font-bold text-zinc-300">{userName.charAt(0).toUpperCase()}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <h1 className="font-raleway text-xl font-semibold text-white">{userName}</h1>
            {verified && (
            ...
            {editingBio ? (
              <div className="flex-1 flex items-center gap-2">
                <input
                  ...
                  className="flex-1 bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-1.5 text-sm font-manrope text-white placeholder:text-zinc-500 focus:outline-none focus:border-fuchsia-500 transition-colors"
                />
                ...
                <button type="button" title="Cancel" onClick={() => { setEditingBio(false); setBioInput(bio); }} className="p-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 transition-colors">
                  <X className="w-3.5 h-3.5" />
                </button>
              </div>
            ) : (
              <>
                <p className="font-manrope text-sm text-zinc-400 flex-1">{bio || "Add a one-line bio…"}</p>
                <button type="button" title="Edit bio" onClick={() => setEditingBio(true)} className="shrink-0 p-1 text-zinc-600 hover:text-zinc-300 transition-colors">
          ...
          <div className="flex items-center gap-4 mt-3 flex-wrap">
            {slug && (
              <Link
                href={`/artists/${slug}`}
                className="inline-flex items-center gap-1.5 text-xs font-manrope text-fuchsia-400 hover:text-fuchsia-300 transition-colors"
              >
            ...
            <Link
              ...
              className="inline-flex items-center gap-1.5 text-xs font-manrope text-zinc-400 hover:text-zinc-200 transition-colors"
            >
              <DollarSign className="w-3.5 h-3.5" />
              <span className="font-semibold text-white">{fmt(totalEarnings)}</span> earned total
            </Link>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {stats.map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="flex items-center gap-4 p-5 rounded-xl border border-zinc-800 bg-zinc-900/60">
            ...
            <div>
              <p className="font-raleway text-xl font-semibold text-white">{value}</p>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-raleway text-base font-semibold text-white">Your Shows</h2>
      </div>

      {shows.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 text-center gap-4 rounded-xl border border-zinc-800 bg-zinc-900/30">
          <div className="w-16 h-16 rounded-2xl bg-zinc-900 border border-zinc-800 flex items-center justify-center">
            <Film className="w-7 h-7 text-zinc-600" />
          </div>
          <div>
            <p className="font-raleway text-white font-semibold text-lg">No shows yet</p>
        ...
        <div className="flex flex-col gap-3">
          {shows.map((show) => {
            ...
            return (
              <div key={show.id} className="flex items-center gap-4 p-4 rounded-xl border border-zinc-800 bg-zinc-900/60 hover:border-zinc-700 transition-colors">
                {/* Thumbnail */}
                <div className="w-20 h-[52px] rounded-lg overflow-hidden bg-zinc-800 shrink-0">
                  {show.thumbnail_url ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <h3 className="font-raleway font-semibold text-white text-sm truncate">{show.title}</h3>
                    {show.featured && (
                ...
                <div className="flex flex-col items-end gap-0.5 shrink-0 w-28">
                  ...
                  <span className="font-manrope text-xs text-emerald-400 font-semibold">{fmt(earned)}</span>
                </div>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {show.status === "published" && (
                    <Link
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-400 text-[11px] font-manrope font-semibold hover:bg-amber-500/20 transition-colors"
                    >
